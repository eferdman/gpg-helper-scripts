...
gpg --batch --passphrase $PASSWORD --quick-gen-key $PRIMARY_UID $MASTER_KEY_ALGO - $MASTER_EXPIRY
...
if ! [ -z $SIGN_SUBKEY ];
then
	gpg --quick-addkey $FPR $SIGN_SUBKEY sign $SIGN_SUBKEY_EXPIRY;
fi

if ! [ -z $ENCR_SUBKEY ];
then
	gpg --quick-addkey $FPR $ENCR_SUBKEY encrypt $ENCR_SUBKEY_EXPIRY;
fi

if ! [ -z $AUTH_SUBKEY ];
then
	gpg --quick-addkey $FPR $AUTH_SUBKEY auth $AUTH_SUBKEY_EXPIRY;
fi
